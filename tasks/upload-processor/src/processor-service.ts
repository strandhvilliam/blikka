...
export class UploadProcessorService extends ServiceMap.Service<UploadProcessorService>()("@blikka/upload-processor/UploadProcessorService", {
  make: Effect.gen(function* () {
    ...
    const handleParticipantError = Effect.fn("UploadProcessorService.handleParticipantError")(
      function* (domain: string, reference: string, errorCode: string, error: Error) {
        yield* Effect.logError(`Failed to set participant error state: ${error.message}`, error.cause)
    ...
    const processPhoto = Effect.fn("UploadProcessorService.processPhoto")(function* (
      ...
      return yield* Effect.gen(function* () {
        ...
        yield* uploadKv
          ...
          .pipe(Effect.catch((error) => Effect.logError("Failed to update submission state", error)))
        ...
        const { finalize } = yield* uploadKv
          ...
          .pipe(
            Effect.catch(
              (error) =>
                Effect.fail(new FailedToIncrementParticipantStateError({
                  cause: error,
        ...
        Effect.annotateLogs({ domain, reference, orderIndex, key })
      )
    })
