...
import type { FileUploadError, PhotoWithPresignedUrl, UploadFileState, UploadPhase } from './types'
...
export const selectCompletedCount = (state: ReturnType<typeof useUploadStore.getState>) =>
  Array.from(state.files.values()).filter((file) => file.phase === UPLOAD_PHASE.UPLOADED).length
