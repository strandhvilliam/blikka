...
describe('participant-upload local save helpers', () => {
  ...
  it('builds one save entry per selected photo', () => {
    ...
    expect(entries.map((entry) => entry.path)).toEqual(['demo/0007/00/a.jpg', 'demo/0007/11/b.jpg'])
  })

  it('creates collision-safe file names without overwriting', () => {
    const existingNames = new Set(['frame.jpg', 'frame-1.jpg'])

    expect(getCollisionSafeFileName('frame.jpg', existingNames)).toBe('frame-2.jpg')
    expect(getCollisionSafeFileName('fresh.jpg', existingNames)).toBe('fresh.jpg')
  })

  it('detects directory picker support', () => {
    expect(supportsDirectoryPicker({ showDirectoryPicker: async () => ({}) as never })).toBe(true)
