...
export function filterRuleConfigsByMarathonMode(
  ...
  return dbRuleConfigs.filter((rule) => !BY_CAMERA_EXCLUDED_RULE_KEYS.has(rule.ruleKey as RuleKey))
}
...
export function hasBlockingValidationErrors(results: ValidationResult[]): boolean {
  return results.some(
...
export function buildValidationInputs(photos: ValidatablePhotoLike[]): ValidationInput[] {
