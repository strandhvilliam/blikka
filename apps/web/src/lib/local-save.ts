...
async function writeEntriesToDirectory(
  ...
  for (const entry of entries) {
    const [domain, participantReference, topicFolder, fileName] = entry.pathSegments
    const rootHandle = await directoryHandle.getDirectoryHandle(domain, {
      create: true,
    })
    const participantHandle = await rootHandle.getDirectoryHandle(participantReference, {
      create: true,
    })
