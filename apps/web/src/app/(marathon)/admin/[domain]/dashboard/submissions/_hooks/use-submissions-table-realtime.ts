...
function reduceTrackingState(
  ...
  let nextProcessed: ReadonlyMap<string, ReadonlySet<number>> | Map<string, ReadonlySet<number>> =
    previous.processed
  ...
  if (nextProcessed === previous.processed && nextFinalized === previous.finalized) {
    return previous
  }
...
export function useSubmissionsTableRealtime({
    ...
    (reference: string) => {
      queryClient.setQueriesData<InfiniteParticipantsData>(
        ...
        (currentData) => {
          ...
          return hasChanges ? { ...currentData, pages: nextPages } : currentData
        },
      )
    },
  ...
  const scheduleFlush = useCallback(() => {
    ...
    flushTimeoutRef.current = setTimeout(flushQueuedEvents, REALTIME_BATCH_WINDOW_MS)
  }, [flushQueuedEvents])
