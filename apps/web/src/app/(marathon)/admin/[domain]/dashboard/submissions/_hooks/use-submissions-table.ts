...
import { getTabQueryParams, normalizeIdArray } from '../_lib/submissions-table-utils'
...
function useSubmissionTableQueryState() {
  ...
  const { tab: activeTab, search, competitionClassId, deviceGroupId } = queryState
...
export function useSubmissionsTable() {
  ...
  const activeByCameraTopicId =
    marathon?.mode === 'by-camera'
      ? (marathon.topics.find((topic) => topic.visibility === 'active')?.id ?? -1)
      : null
