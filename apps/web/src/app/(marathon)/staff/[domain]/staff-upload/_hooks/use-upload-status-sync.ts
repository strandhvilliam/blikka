  ...
  useEffect(() => {
    ...
    const updatedFiles = uploadFiles.map((file) => {
      ...
      if (status?.uploaded && file.phase !== PARTICIPANT_UPLOAD_PHASE.COMPLETED) {
        return { ...file, phase: PARTICIPANT_UPLOAD_PHASE.COMPLETED, progress: 100, error: undefined }
      }

      return file
    })

    if (updatedFiles !== uploadFiles) {
      patchUpload({ uploadFiles: updatedFiles })
    }

    if (uploadStatusData.participant?.errors.length) {
      patchUpload({ uploadErrorMessage: uploadStatusData.participant.errors.join(", ") })
    }
