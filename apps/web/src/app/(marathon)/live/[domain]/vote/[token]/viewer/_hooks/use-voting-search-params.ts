...
export function useVotingSearchParams() {
  ...
  return {
    ...
    setCurrentFilter: (filter: number | null) => setParams((prev) => ({ ...prev, filter })),
    currentImageIndex: params.image,
    viewMode: params.view,
    setCurrentImageIndex: (index: number) => setParams((prev) => ({ ...prev, image: index })),
    setViewMode: (view: 'carousel' | 'grid') => setParams((prev) => ({ ...prev, view })),
    setParams,
  }
}
