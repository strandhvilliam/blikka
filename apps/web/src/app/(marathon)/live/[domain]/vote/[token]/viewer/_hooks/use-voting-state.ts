  ...
  create<VotingStore>()(
    persist(
      (set) => ({
        ...
        setSelectedSubmission: (submissionId) => set({ selectedSubmissionId: submissionId }),
      }),
      {
        name: storageKey,
...
export function useVotingState({ token, domain, storageKey }: UseVotingStateOptions) {
  const trpc = useTRPC()
  ...
  const useVotingStore = useMemo(() => getVotingStore(resolvedStorageKey), [resolvedStorageKey])

  const ratings = useVotingStore((state) => state.ratings)
  const selectedSubmissionId = useVotingStore((state) => state.selectedSubmissionId)
  const setRating = useVotingStore((state) => state.setRating)
  const setSelectedSubmission = useVotingStore((state) => state.setSelectedSubmission)
  ...
  const getFilteredSubmissions = useCallback(
    (filterRating: number | null) => {
      ...
      return submissions.filter((s) => ratings[s.submissionId] === filterRating)
    },
    [submissions, ratings],
  )
