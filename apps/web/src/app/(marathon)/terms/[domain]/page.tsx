...
const getTermsMarkdown = async function getTermsMarkdown(domain: string) {
  ...
  const result = await Effect.runPromiseExit(
    Effect.gen(function* () {
      const bucket = yield* Config.string(
        "NEXT_PUBLIC_MARATHON_SETTINGS_BUCKET_NAME",
      );
      ...
      if (!response.ok)
        return yield* Effect.fail(
          new Error(`Response result was not ok. ${response.status}`),
        );
  ...
  return (
    <main className="mx-auto min-h-svh w-full max-w-3xl px-4 py-10 sm:px-6 sm:py-12">
      <TermsHero
        ...
        logoUrl={publicMarathon?.logoUrl}
      />
