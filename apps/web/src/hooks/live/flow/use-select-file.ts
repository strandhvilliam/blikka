...
interface UseSelectFileResult {
  handleFileSelect: (fileList: FileList | null, replace?: boolean) => Promise<void>
}

export function useSelectFile({ maxPhotos, t }: UseSelectFileOptions): UseSelectFileResult {
  const photos = usePhotoStore((state) => state.photos)
  const setPhotos = usePhotoStore((state) => state.setPhotos)
  const setIsProcessingFiles = usePhotoStore((state) => state.setIsProcessingFiles)
  ...
  const handleFileSelect = useCallback(
      ...
      try {
        ...
        if (
          result.warnings.some(
            (message) =>
              message.startsWith('Only ') && message.endsWith(' additional image(s) accepted'),
          )
        ) {
    ...
    [photos, maxPhotos, convertFiles, setIsProcessingFiles, setPhotos, t, topicOrderIndexes],
  )
