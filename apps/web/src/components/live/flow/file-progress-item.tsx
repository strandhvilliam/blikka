...
export function FileProgressItem({ file, topic }: FileProgressItemProps) {
  ...
  const errorPresentation = file.error ? getUploadErrorPresentation(file.error) : null
    ...
    >
      <div className="flex-1 min-w-0 mr-3">
        ...
        <p
          className={cn(
            'text-xs',
            file.phase === UPLOAD_PHASE.ERROR ? 'text-destructive' : 'text-muted-foreground',
          )}
        >
          {getStatusText()}
        </p>
        {file.phase === UPLOAD_PHASE.ERROR && errorPresentation && (
          <>
            <p className="mt-1 text-xs text-muted-foreground">{t(errorPresentation.bodyKey)}</p>
            {errorPresentation.actionKey && (
              <p className="mt-1 text-xs text-muted-foreground">{t(errorPresentation.actionKey)}</p>
            )}
