        ...
        <div className="flex items-center justify-center">
          {[1, 2, 3, 4, 5].map((rating) => (
            <div key={rating} className="flex flex-col items-center gap-1 px-2">
              ...
              <span className="text-[10px] font-medium text-muted-foreground/40">{rating}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className={cn('space-y-2', className)}>
      <div
        ...
        onMouseLeave={() => (!isMobile && !disabled ? setHoverValue(null) : undefined)}
      >
        {[1, 2, 3, 4, 5].map((rating) => (
          <button
            key={rating}
            ...
            onMouseEnter={() => (!isMobile && !disabled ? setHoverValue(rating) : undefined)}
            onTouchStart={() => (!isMobile && !disabled ? setHoverValue(rating) : undefined)}
            className={cn(
          ...
          >
            ...
            <span
              className={cn(
                ...
                displayValue >= rating ? 'text-yellow-500' : 'text-muted-foreground/40',
              )}
            >
              {rating}
            </span>
