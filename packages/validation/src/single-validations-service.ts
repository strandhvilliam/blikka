...
export class SingleValidationsService extends ServiceMap.Service<SingleValidationsService>()(
  "@blikka/packages/validation/SingleValidationsService",
  {
    make: Effect.gen(function* () {
      const validateMaxFileSize = (params: RuleParams["max_file_size"], input: ValidationInput) =>
        Effect.gen(function* () {
      ...
      const validateAllowedFileTypes = (
        ...
        Effect.gen(function* () {
          ...
          const parsedAllowedFileTypes = params.allowedFileTypes.reduce((acc, curr) => {
            if (curr === "jpeg") {
              acc.push("jpg")
            }
            acc.push(curr)
          ...
          const filteredMimeTypes = Object.entries(IMAGE_EXTENSION_TO_MIME_TYPE).filter(
            ([key, _]) => params.allowedFileTypes.includes(key),
          )
          ...
          const isValidMimeType = filteredMimeTypes.some(([_, value]) => value === input.mimeType)
          ...
          return yield* Effect.succeed("File is valid and within the allowed file types")
        })
      const validateTimeframe = (params: RuleParams["within_timerange"], input: ValidationInput) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.succeed("Photo was taken within the specified timeframe")
        })
      const validateModified = (params: RuleParams["modified"], input: ValidationInput) =>
        Effect.gen(function* () {
            ...
            if (isEdited) {
              return yield* new ValidationFailure({
                ...
                context: {
                  ...
                  timeDifferenceHours: (modifyTime - createTime) / (60 * 60 * 1000),
                },
              })
            }
          }
