...
import {
  ResendEffectClient,
  ResendEffectClientLayer,
} from "./resend-effect-client"
...
const makeEmailService = Effect.gen(function* () {
  ...
  const send: EmailService["Service"]["send"] = Effect.fn("EmailService.send")(
  ...
  const sendBatch: EmailService["Service"]["sendBatch"] = Effect.fn(
...
export const EmailServiceLayerNoDeps = Layer.effect(
  EmailService,
  makeEmailService,
)
