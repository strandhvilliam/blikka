...
import { normalizeUploadContentType } from "./contracts"
...
export class UploadFlowService extends Context.Service<UploadFlowService>()(
  "@blikka/api/UploadFlowService",
  {
    make: Effect.gen(function* () {
      ...
      const resetAndSeedUploadExif = Effect.fn("UploadFlowService.resetAndSeedUploadExif")(
        function* ({
          domain,
          reference,
          staleOrderIndexes,
      ...
      const getActiveByCameraTopicOrFail = Effect.fn(
        "UploadFlowService.getActiveByCameraTopicOrFail",
      )(function* ({
        domain,
        marathon,
      }: {
      ...
      const checkParticipantExists = Effect.fn("UploadFlowService.checkParticipantExists")(
      ...
      const prepareUploadFlow = Effect.fn("UploadFlowService.prepareUploadFlow")(function* ({
        domain,
        reference,
        firstname,
        lastname,
      ...
      const initializeUploadFlow = Effect.fn("UploadFlowService.initializeUploadFlow")(
        function* ({
          domain,
          reference,
          firstname,
      ...
      const resolveByCameraParticipantByPhone = Effect.fn(
        "UploadFlowService.resolveByCameraParticipantByPhone",
      ...
      const getUploadStatus = Effect.fn("UploadFlowService.getUploadStatus")(function* ({
        domain,
        reference,
        orderIndexes,
      }) {
      ...
      const initializeByCameraUpload = Effect.fn("UploadFlowService.initializeByCameraUpload")(
        function* ({
          domain,
          firstname,
          lastname,
      ...
      const initializeStaffByCameraUpload = Effect.fn(
        "UploadFlowService.initializeStaffByCameraUpload",
      )(function* ({
        domain,
        reference,
        firstname,
      ...
      const refreshPresignedUploads = Effect.fn("UploadFlowService.refreshPresignedUploads")(
      ...
      const reTriggerUploadFlow = Effect.fn("UploadFlowService.reTriggerUploadFlow")(function* ({
        domain,
        reference,
      }) {
        const participantState = yield* kv.getParticipantState(domain, reference)
