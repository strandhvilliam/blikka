...
type ParticipantVerificationByReference = Effect.Success<
  ReturnType<
    ValidationsRepository["Service"]["getParticipantVerificationByReference"]
  >
>
...
export class ValidationsService extends Context.Service<
  ValidationsService,
  {
    ...
    readonly runValidations: (input: RunValidations) => Effect.Effect<
    ...
    readonly updateValidationResult: (input: UpdateValidationResult) => Effect.Effect<UpdatedValidationResultRow, DbError, never>
...
const makeValidationsService = Effect.gen(function* () {
  ...
  const runValidations: ValidationsService["Service"]["runValidations"] =
    Effect.fn("ValidationsService.runValidations")(function* ({
      domain,
      reference,
    }) {
      ...
      const validationRules = yield* Effect.forEach(
        ...
        (rule) =>
          Effect.gen(function* () {
            const validationRuleKey = yield* Schema.decodeUnknownEffect(
              RuleKeySchema,
            )(rule.ruleKey)
  ...
  const updateValidationResult: ValidationsService["Service"]["updateValidationResult"] =
    Effect.fn("ValidationsService.updateValidationResult")(function* ({
      id,
      data,
    }) {
  ...
  const getParticipantVerificationByReference: ValidationsService["Service"]["getParticipantVerificationByReference"] =
    Effect.fn("ValidationsService.getParticipantVerificationByReference")(
