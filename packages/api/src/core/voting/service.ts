...
export class VotingService extends Context.Service<
  VotingService,
  {
     ...
     */
    readonly createManualVotingSession: (
      input: CreateManualVotingSession,
    ) => Effect.Effect<
      {
        session: {
          id: number
...
const makeVotingService = Effect.gen(function* () {
  ...
  const updateNotificationTimestamp = Effect.fn(
    "VotingService.updateNotificationTimestamp",
  ...
  const sendVotingInviteEmails = Effect.fn(
    "VotingService.sendVotingInviteEmails",
  )(function* ({
    marathonName,
    marathonLogoUrl,
    domain,
  ...
  const generateUniqueToken = Effect.fn("VotingService.generateUniqueToken")(
    function* ({
  ...
  const getTopicVotingWindow = Effect.fn("VotingService.getTopicVotingWindow")(
    function* ({
      marathonId,
      topicId,
    }: {
  ...
  const getVotingSession: VotingService["Service"]["getVotingSession"] =
    Effect.fn("VotingService.getVotingSession")(function* ({ token }) {
      const votingSessionResult =
        yield* votingRepository.getVotingSessionByToken({ token })
  ...
  const reopenTopicVotingWindow: VotingService["Service"]["reopenTopicVotingWindow"] =
    Effect.fn("VotingService.reopenTopicVotingWindow")(function* ({
      domain,
      topicId,
    }) {
      const { marathon, topic, activeTopic } =
  ...
  const startTiebreakRound: VotingService["Service"]["startTiebreakRound"] =
    Effect.fn("VotingService.startTiebreakRound")(function* ({
      domain,
      topicId,
      endsAt,
    }) {
  ...
  const startVotingSessions: VotingService["Service"]["startVotingSessions"] =
    Effect.fn("VotingService.startVotingSessions")(function* ({
      domain,
      topicId,
      endsAt,
      sendInitialSms,
  ...
  const getParticipantsWithoutVotingSession: VotingService["Service"]["getParticipantsWithoutVotingSession"] =
    Effect.fn("VotingService.getParticipantsWithoutVotingSession")(function* ({
      domain,
      topicId,
    }) {
  ...
  const startVotingSessionsForParticipants: VotingService["Service"]["startVotingSessionsForParticipants"] =
    Effect.fn("VotingService.startVotingSessionsForParticipants")(function* ({
      ...
      const fullParticipants = yield* Effect.all(
        idsToProcess.map((id) =>
          participantsRepository.getParticipantById({ id }),
        ),
      )
  ...
  const getSubmissionVoteStats: VotingService["Service"]["getSubmissionVoteStats"] =
    Effect.fn("VotingService.getSubmissionVoteStats")(function* ({
      submissionId,
      domain,
    }) {
  ...
  const getVotingAdminSummary: VotingService["Service"]["getVotingAdminSummary"] =
    Effect.fn("VotingService.getVotingAdminSummary")(function* ({
      domain,
      topicId,
    }) {
  ...
  const getVotingRoundsForTopic: VotingService["Service"]["getVotingRoundsForTopic"] =
    Effect.fn("VotingService.getVotingRoundsForTopic")(function* ({
      domain,
      topicId,
    }) {
      const { marathon } = yield* getByCameraMarathonWithTopic({
  ...
  const getVotingLeaderboardPage: VotingService["Service"]["getVotingLeaderboardPage"] =
    Effect.fn("VotingService.getVotingLeaderboardPage")(function* ({
      domain,
      topicId,
      page,
      limit,
  ...
  const getVotingVotersPage: VotingService["Service"]["getVotingVotersPage"] =
    Effect.fn("VotingService.getVotingVotersPage")(function* ({
      domain,
      topicId,
      page,
      limit,
  ...
  const getVotingSubmissions: VotingService["Service"]["getVotingSubmissions"] =
    Effect.fn("VotingService.getVotingSubmissions")(function* ({ token }) {
      const votingSessionResult =
        yield* votingRepository.getVotingSessionByToken({ token })
  ...
  const submitVote: VotingService["Service"]["submitVote"] = Effect.fn(
    "VotingService.submitVote",
  )(function* ({ token, submissionId }) {
    const votingSessionResult = yield* votingRepository.getVotingSessionByToken(
      { token },
    )
  ...
  const clearVote: VotingService["Service"]["clearVote"] = Effect.fn(
    "VotingService.clearVote",
  ...
  const deleteVotingSession: VotingService["Service"]["deleteVotingSession"] =
    Effect.fn("VotingService.deleteVotingSession")(function* ({
      domain,
      topicId,
      sessionId,
    }) {
      ...
      const sessionResult =
        yield* votingRepository.getVotingSessionByIdForTopic({
          marathonId: marathon.id,
          topicId,
          sessionId,
        })
...
export const VotingServiceLayerNoDeps = Layer.effect(
  VotingService,
  makeVotingService,
)
