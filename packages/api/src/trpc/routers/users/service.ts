...
export class UsersApiService extends Context.Service<UsersApiService>()(
  "@blikka/api/UsersApiService",
  {
    make: Effect.gen(function* () {
      ...
      const getMarathonIdByDomain = Effect.fn(
        "UsersApiService.getMarathonIdByDomain",
      ...
      const getStaffMembersByDomain = Effect.fn(
        "UsersApiService.getStaffMembersByDomain",
      )(function* ({ domain }: { domain: string }) {
        return yield* usersRepository.getStaffMembersByDomain({ domain });
      });

      const getStaffAccessById = Effect.fn(
        "UsersApiService.getStaffAccessById",
      )(function* ({ accessId, domain }: { accessId: string; domain: string }) {
      ...
      const createStaffMember = Effect.fn("UsersApiService.createStaffMember")(
        function* ({
          domain,
          data,
        }: {
      ...
      const deleteStaffAccess = Effect.fn("UsersApiService.deleteStaffAccess")(
        function* ({ domain, accessId }: { domain: string; accessId: string }) {
          const parsed = parseAccessId(accessId);
      ...
      const getVerificationsByStaffId = Effect.fn(
        "UsersApiService.getVerificationsByStaffId",
      )(function* ({
        staffId,
        domain,
        cursor,
      ...
      const updateStaffAccess = Effect.fn("UsersApiService.updateStaffAccess")(
        function* ({
          accessId,
          domain,
          data,
  ...
  static readonly layer = Layer.effect(this, this.make).pipe(
    Layer.provide(Layer.mergeAll(DbLayer, RedisClient.layer)),
  );
}
