...
import { DbLayer, ZippedSubmissionsRepository } from "@blikka/db";
import {
...
export class ZipFilesApiService extends Context.Service<ZipFilesApiService>()(
  "@blikka/api/ZipFilesApiService",
  {
    make: Effect.gen(function* () {
      ...
      const getZipSubmissionStats = Effect.fn(
        "ZipFilesApiService.getZipSubmissionStats",
      ...
      const getZipDownloadProgress = Effect.fn(
        "ZipFilesApiService.getZipDownloadProgress",
      ...
      const getActiveProcess = Effect.fn("ZipFilesApiService.getActiveProcess")(
        function* ({ domain }: { domain: string }) {
      ...
      const cancelDownloadProcess = Effect.fn(
        "ZipFilesApiService.cancelDownloadProcess",
      )(function* ({
        domain,
        processId,
      }: {
      ...
      const getZipDownloadUrls = Effect.fn(
        "ZipFilesApiService.getZipDownloadUrls",
      )(function* ({ processId }: { processId: string }) {
        const processStateOption =
          yield* downloadStateRepository.getDownloadProcess(processId);
        if (Option.isNone(processStateOption)) {
      ...
      const initializeZipDownloads = Effect.fn(
        "ZipFilesApiService.initializeZipDownloads",
      )(function* ({ domain }: { domain: string }) {
        try {
          const zips =
          ...
          for (const zips of Object.values(byCompetitionClass)) {
            ...
            const sortedZips = zips.sort(
              (a, b) =>
                Number(a.participant.reference) -
                Number(b.participant.reference),
            );
          ...
          yield* Effect.forEach(Object.values(byCompetitionClass), (zips) =>
            Effect.gen(function* () {
              ...
              const competitionClassId =
                zips[0].participant.competitionClass!.id;
              const competitionClassName = zips[0].participant
                .competitionClass!.name.toLowerCase()
                .replace(/ /g, "-");
              ...
              yield* Effect.forEach(
                ...
                (chunk, index) =>
                  Effect.gen(function* () {
                    ...
                    const minParticipantReference =
                      chunk[0]?.participant.reference;
                    ...
                    const minRefPadded = minParticipantReference.padStart(
                      4,
                      "0",
                    );
                    const maxRefPadded = maxParticipantReference.padStart(
                      4,
                      "0",
                    );
                    ...
                    yield* Effect.logInfo({
                      message:
                        "Chunk state saved to Redis and added to process",
                      processId,
                      jobId,
                      domain,
                      competitionClassId,
  ...
  static readonly layer = Layer.effect(this, this.make).pipe(
    Layer.provide(
      Layer.mergeAll(DbLayer, DownloadStateRepositoryLayer, S3Service.layer),
    ),
  );
}
