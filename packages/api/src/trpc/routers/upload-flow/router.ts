...
export const uploadFlowRouter = createTRPCRouter({
  ...
  prepareUploadFlow: publicProcedure.input(PrepareUploadFlowSchema).mutation(
    trpcEffect(
      Effect.fn("UploadFlowRouter.prepareUploadFlow")(function* ({ input }) {
        return yield* UploadFlowApiService.use((s) => s.prepareUploadFlow(input))
      }),
    ),
  ),

  initializeByCameraUpload: publicProcedure.input(InitializeByCameraUploadSchema).mutation(
    trpcEffect(
    ...
    .mutation(
      trpcEffect(
        Effect.fn("UploadFlowRouter.initializeStaffByCameraUpload")(function* ({ input }) {
          return yield* UploadFlowApiService.use((s) => s.initializeStaffByCameraUpload(input))
        }),
      ),
    ),
    ...
    .mutation(
      trpcEffect(
        Effect.fn("UploadFlowRouter.resolveByCameraParticipantByPhone")(function* ({ input }) {
          return yield* UploadFlowApiService.use((s) => s.resolveByCameraParticipantByPhone(input))
        }),
      ),
    ),

  checkParticipantExists: publicProcedure.input(CheckParticipantExistsSchema).mutation(
    trpcEffect(
  ...
  refreshPresignedUploads: publicProcedure.input(RefreshPresignedUploadsSchema).mutation(
    trpcEffect(
      Effect.fn("UploadFlowRouter.refreshPresignedUploads")(function* ({ input }) {
        return yield* UploadFlowApiService.use((s) => s.refreshPresignedUploads(input))
      }),
    ),
  ),
