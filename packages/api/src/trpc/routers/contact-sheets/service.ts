...
export class ContactSheetsApiService extends Context.Service<ContactSheetsApiService>()(
  "@blikka/api/contact-sheets-api-service",
  {
    make: Effect.gen(function* () {
      ...
      const validatePhotoCount = Effect.fn(
        "ContactSheetsApiService.validatePhotoCount",
      )(function* (
        reference: string,
        domain: string,
        keys: string[],
      ...
      const generateContactSheet = Effect.fn(
        "ContactSheetsApiService.generateContactSheet",
  ...
  static readonly layer = Layer.effect(this, this.make).pipe(
    Layer.provide(
      Layer.mergeAll(DbLayer, S3Service.layer, ContactSheetBuilder.layer),
    ),
  );
}
