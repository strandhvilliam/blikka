  ...
  const stateLabel =
    outcome === "passed"
      ...
      : severity === "warning"
        ? "Warning"
        : "Error"
  const topicLabel = plan ? `topic ${plan.topic.orderIndex + 1} (${plan.topic.name})` : "submission set"
...
const buildSubmissionAssets = Effect.fn("SeedService.buildSubmissionAssets")(function* ({
  topic,
  participant,
  mode,
}: {
...
const getSortedStaffMembers = Effect.fn("SeedService.getSortedStaffMembers")(function* ({
  domain,
}: {
  domain: string
}) {
  const db = yield* Database
  const staffMembers = yield* db.usersQueries.getStaffMembersByDomain({ domain })
...
const getMarathonOrFail = Effect.fn("SeedService.getMarathonOrFail")(function* ({
  domain,
}: {
  domain: string
}) {
...
export const getSeedScenarioStatus = Effect.fn("SeedService.getSeedScenarioStatus")(function* ({
  domain,
  isAdminForDomain,
}: {
  domain: string
...
const createDeviceGroups = Effect.fn("SeedService.createDeviceGroups")(function* ({
  marathonId,
}: {
  marathonId: number
}) {
...
const createCompetitionClasses = Effect.fn("SeedService.createCompetitionClasses")(function* ({
  marathonId,
}: {
  marathonId: number
}) {
  const db = yield* Database
  const createdClasses = yield* db.competitionClassesQueries.createMultipleCompetitionClasses({
    data: [
      {
        marathonId,
        name: "8 Images",
...
const createSeedParticipants = Effect.fn("SeedService.createSeedParticipants")(function* ({
  domain,
  marathonId,
  mode,
...
const uploadSubmissionAssets = Effect.fn("SeedService.uploadSubmissionAssets")(function* ({
  domain,
  participant,
  topic,
  createdAt,
...
const createSeedSubmissions = Effect.fn("SeedService.createSeedSubmissions")(function* ({
  domain,
  participants,
  topics,
  mode,
  ...
  const createdSubmissions = yield* db.submissionsQueries.createMultipleSubmissions({
...
const syncWithinTimerangeRule = Effect.fn("SeedService.syncWithinTimerangeRule")(function* ({
  domain,
  startDate,
  endDate,
}: {
...
const createSeedValidationResults = Effect.fn("SeedService.createSeedValidationResults")(function* ({
  domain,
  participants,
  plans,
  rules,
...
const createParticipantVerifications = Effect.fn("SeedService.createParticipantVerifications")(
  function* ({
    participants,
    staffMembers,
  }: {
...
const createContactSheets = Effect.fn("SeedService.createContactSheets")(function* ({
  domain,
  participants,
  plans,
  topics,
...
const createJuryInvitationsAndRatings = Effect.fn("SeedService.createJuryInvitationsAndRatings")(
  function* ({
    domain,
    participants,
    now,
...
const createVotingSessions = Effect.fn("SeedService.createVotingSessions")(function* ({
  participants,
  activeTopic,
  submissions,
  now,
...
export const seedFinishedScenario = Effect.fn("SeedService.seedFinishedScenario")(function* ({
  domain,
  isAdminForDomain,
}: {
  domain: string
