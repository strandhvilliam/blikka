...
export class ParticipantsRepository extends Context.Service<ParticipantsRepository>()(
  "@blikka/db/participants-repository",
  {
    make: Effect.gen(function* () {
      ...
      const getParticipantByReference = Effect.fn(
        "ParticipantsRepository.getParticipantByReferenceQuery",
      )(function* ({
        reference,
        domain,
      }: {
      ...
      const getByPhoneHashForByCamera = Effect.fn(
        "ParticipantsRepository.getByPhoneHashForByCameraQuery",
      )(function* ({
        marathonId,
        phoneHash,
      }: {
      ...
      const getInfiniteParticipantsByDomain = Effect.fn(
        "ParticipantsRepository.getInfiniteParticipantsByDomainQuery",
      )(function* ({
        domain,
        cursor,
        limit = 50,
              ...
              submissionHealth:
                topicId === undefined
                  ...
                  : {
                      ...
                      hasThumbnail:
                        latestTopicSubmission?.thumbnailKey !== null,
                    },
              votingSession:
                votingSessions
                  .filter((session) =>
      ...
      const updateParticipantById = Effect.fn(
        "ParticipantsRepository.updateParticipantMutation",
      )(function* ({
        id,
        data,
      }: {
      ...
      const updateParticipantByReference = Effect.fn(
        "ParticipantsRepository.updateParticipantByReference",
      )(function* ({
        reference,
        domain,
        data,
      ...
      const createTermsAcceptance = Effect.fn(
        "ParticipantsRepository.createTermsAcceptance",
      )(function* ({ data }: { data: NewParticipantTermsAcceptance }) {
        const [result] = yield* use((db) =>
          db
            .insert(participantTermsAcceptances)
            .values(data)
            .onConflictDoNothing()
            .returning(),
        );
      ...
      const batchDeleteParticipants = Effect.fn(
        "ParticipantsRepository.batchDeleteParticipants",
      ...
      const batchVerifyParticipants = Effect.fn(
        "ParticipantsRepository.batchVerifyParticipants",
      )(function* ({ ids, domain }: { ids: number[]; domain: string }) {
        if (ids.length === 0) {
      ...
      const batchMarkParticipantsCompleted = Effect.fn(
        "ParticipantsRepository.batchMarkParticipantsCompleted",
      )(function* ({ ids, domain }: { ids: number[]; domain: string }) {
        if (ids.length === 0) {
          return { updatedCount: 0, failedIds: [] };
        }
