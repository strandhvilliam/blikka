...
export class ParticipantsQueries extends ServiceMap.Service<ParticipantsQueries>()(
  "@blikka/db/participants-queries",
  {
    make: Effect.gen(function* () {
      ...
      )(function* ({ marathonId, phoneHash }: { marathonId: number; phoneHash: string }) {
        const result = yield* use((db) =>
          db.query.participants.findFirst({
            where: (table, operators) =>
              operators.and(
      ...
      const getDashboardOverview = Effect.fn("ParticipantsQueries.getDashboardOverview")(
      ...
      const createTermsAcceptance = Effect.fn("ParticipantsQueries.createTermsAcceptance")(
      ...
      const batchDeleteParticipants = Effect.fn("ParticipantsQueries.batchDeleteParticipants")(
      ...
      const batchVerifyParticipants = Effect.fn("ParticipantsQueries.batchVerifyParticipants")(
        function* ({ ids, domain }: { ids: number[]; domain: string }) {
          if (ids.length === 0) {
  ...
  static readonly layer = Layer.effect(this, this.make).pipe(Layer.provide(DrizzleClient.layer))
}
