...
const makeValidationRunner = Effect.gen(function* () {
  ...
  const execute = Effect.fn("ValidationRunner.execute")(
    function* ({
      ...
      const validationInputs = yield* createValidationInputs(
        exifStates,
        submissionStates,
      );
      const validationRules = yield* createValidationRules(rules);
      const validationResults = yield* engine.runValidations(
        validationRules,
        validationInputs,
      );
...
export const ValidationRunnerLayerNoDeps = Layer.effect(
  ValidationRunner,
  makeValidationRunner,
);
