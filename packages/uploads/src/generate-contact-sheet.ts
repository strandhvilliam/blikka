...
export const generateContactSheet = Effect.fn("ContactSheetGenerator.generateContactSheet")(
  function* (params: GenerateContactSheetParams) {
    ...
    return yield* Effect.gen(function* () {
      ...
      if (!sessionGuard.matched) {
        yield* Effect.logWarning("Dropping contact sheet event for non-current upload session", {
      ...
      const topics = yield* db.topicsQueries.getTopicsByDomain({ domain: params.domain }).pipe(
        Effect.map((topics) =>
      ...
      const contactSheetKey = generateContactSheetKey(params.domain, params.reference, timestamp)
