...
export class DownloadStateRepository extends Context.Service<DownloadStateRepository>()(
  "@blikka/packages/kv-store/download-state-repository",
  {
    make: Effect.gen(function* () {
      ...
      const saveChunkState = Effect.fn(
        "DownloadStateRepository.saveChunkState",
      )(function* (jobId: string, state: ChunkState) {
      ...
      const getChunkState = Effect.fn("DownloadStateRepository.getChunkState")(
      ...
      const createDownloadProcess = Effect.fn(
        "DownloadStateRepository.createDownloadProcess",
      ...
      const getDownloadProcess = Effect.fn(
      ...
      const updateDownloadProcess = Effect.fn(
        "DownloadStateRepository.updateDownloadProcess",
      )(function* (
        processId: string,
        updates: Partial<
          Omit<DownloadProcessState, "processId" | "domain" | "createdAt">
        >,
      ) {
        yield* Effect.logInfo({
        ...
        if (!exists) {
          return yield* Effect.fail(
            new Error(`Download process not found: ${processId}`),
          );
        }
      ...
      const atomicIncrementCompleted = Effect.fn(
        ...
        const result = yield* Effect.tryPromise({
          try: () =>
            ...
            new Error(
              `Failed to increment completed chunks: ${error instanceof Error ? error.message : String(error)}`,
            ),
        });
       ...
       * This is safe to call concurrently from multiple ECS tasks.
       */
      const atomicIncrementFailed = Effect.fn(
        "DownloadStateRepository.atomicIncrementFailed",
      )(function* (
        processId: string,
        totalChunks: number,
        failedJobId: string,
        ...
        const result = yield* Effect.tryPromise({
          try: () =>
            ...
            new Error(
              `Failed to increment failed chunks: ${error instanceof Error ? error.message : String(error)}`,
            ),
        });
       ...
       */
      const addJobToProcess = Effect.fn(
        "DownloadStateRepository.addJobToProcess",
      )(function* (processId: string, jobId: string) {
        const key = Keys.downloadProcess(processId);
        const now = new Date().toISOString();
        ...
        const result = yield* Effect.tryPromise({
          try: () =>
            ...
            new Error(
              `Failed to add job to process: ${error instanceof Error ? error.message : String(error)}`,
            ),
        });
       ...
       */
      const cancelDownloadProcess = Effect.fn(
        "DownloadStateRepository.cancelDownloadProcess",
      )(function* (processId: string) {
        const key = Keys.downloadProcess(processId);
        const now = new Date().toISOString();
       ...
       */
      const isProcessActive = Effect.fn(
